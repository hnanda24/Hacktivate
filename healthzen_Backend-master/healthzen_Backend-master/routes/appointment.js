...
router.post("/bookAppointment/:id", fetchuser, async (req, res) => {
    try { 
        ...
        if (user.user !== req.user) {
          ...
          const patientName = await Patient.findById(req.patient).select("name");
        ...
        //   appointment = await Appointment.findById()
          let patientUser = await Patient.updateOne({_id:req.patient},{ $push:{appointments:appointment.id}})
...
router.get("/getAllAppointments", fetchuser, async (req, res) => {
    try {
      const patient = await Appointment.find({"patientId":req.patient}).select("");
      // console.log(patient);
